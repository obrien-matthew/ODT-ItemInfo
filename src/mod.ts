		...
		for (const itemID in this.items) {
			...
			if (
				...
				if (item._props.StackMaxSize > 1) {
					itemTraderValuePerSlot = itemTraderValuePerSlot * item._props.StackMaxSize
					itemFleaValuePerSlot = itemFleaValuePerSlot * item._props.StackMaxSize
				}
				if (itemSlots > 1) {
				...
				if (isKey) {
					...
					if (isAmmo) {
						...
						if (config.BulletStatsInName.enabled) {
							...
							this.addToName(itemID, ` (${item._props.Damage * damageMult}/${pen})`, "append")
						}
					} else {
						const ammo = this.items[item._props.StackSlots[0]._props.filters[0].Filter[0]]
						...
						if (config.BulletStatsInName.enabled) {
							...
							this.addToName(itemID, ` (${ammo._props.Damage * damageMult}/${pen})`, "append")
						}
					}
					if (pen >= 60) {
						item._props.BackgroundColor = tiers.backgroundColors[0]
				...
				if (config.ArmorInfo.enabled) {
					if (item._props.armorClass > 0) {
						...
						this.addToName(itemID, `[L${item._props?.armorClass}] `, "prepend")
						this.addToShortName(itemID, ` [L${item._props?.armorClass}]`, "append")
					}
				}
				...
				if (config.ContainerInfo.enabled) {
					if (item._props.Grids?.length > 0) {
						...
						this.addToName(itemID, ` [${totalSlots}, x${slotefficiency}]`, "append")
						this.addToShortName(itemID, `[${containerSize}, ${totalSlots}, x${slotefficiency}] `, "prepend")
					}
				}
				...
				if (config.PricesInfo.enabled) {
					...
					// 	}
					// }
					priceString += (config.PricesInfo.addFleaPrice && fleaPrice > 0 ? this.formatPrice(fleaPrice) + " ₽ [" + this.formatPrice(itemFleaValuePerSlot) + " ₽/s] {" + this.formatPrice(itemFleaValuePerSlotPerKg) + " ₽/s/kg} (Flea)" + newLine : "") 
					priceString += this.formatPrice(traderPrice) + " ₽ [" + this.formatPrice(itemTraderValuePerSlot) + " ₽/s] {" + this.formatPrice(itemTraderValuePerSlotPerKg) + " ₽/s/kg} (" + traderName + ")" + newLine;
					priceString += newLine
				}
				...
				if (config.BarterInfo.enabled) {
					if (barterInfo.barters.length > 1) {
						barterString = "BARTER FOR" + newLine + "---------" + newLine + barterInfo.barters + newLine
					}
				}

				if (config.BarterResourceInfo.enabled) {
					if (barterResourceInfo.length > 1) {
						usedForBarterString = "BARTER WITH" + newLine + "---------" + newLine + barterResourceInfo + newLine
					}
				}
				...
				if (config.ProductionInfo.enabled) {
					...
					if (productionInfo.length > 1) {
						productionString = "PRODUCTION" + newLine + "---------" + newLine + productionInfo + newLine
					}
				}
				...
				if (config.QuestInfo.enabled) {
					...
					if (itemQuestInfo.length > 1) {
						usedForQuestsString = "QUESTS" + newLine + "---------" + newLine + itemQuestInfo + newLine
				...
				if (config.HideoutInfo.enabled) {
					...
					if (itemHideoutInfo.length > 1) {
						usedForHideoutString = "HIDEOUT" + newLine + "---------" + newLine + itemHideoutInfo + newLine
					}
				}
				...
				if (config.CraftingMaterialInfo.enabled) {
					...
					if (itemCraftingMaterialInfo.length > 1) {
						usedForCraftingString = "CRAFTING" + newLine + "---------" + newLine + itemCraftingMaterialInfo + newLine
					}
				}
